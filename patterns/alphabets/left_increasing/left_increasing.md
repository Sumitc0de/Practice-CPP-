...
Print a left aligned alphabet triangle where every row starts from A and increases up to the row length.
...
The column number j decides the character, so each row prints A, then B, then C, up to its own length.
