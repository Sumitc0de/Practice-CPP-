...
Print a left aligned star triangle where row i contains i + 1 stars in the zero based loop.
...
The outer loop is zero based and the inner loop runs while j < i + 1, so rows contain 1 through n stars.
...
- Main concepts: Nested Loops, Triangle, Star Pattern.
