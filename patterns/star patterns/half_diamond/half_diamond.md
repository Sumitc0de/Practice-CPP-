...
Print a left aligned half diamond by first increasing star count and then decreasing it.
...
The first loop prints 1 to n stars. The second loop prints n - 1 down to 0 stars because its limit is n - i.
...
- Main concepts: Nested Loops, Half Diamond, Increasing Pattern.
...
- `Upper half increases by row.`
- `Lower half decreases by row.`
- `The middle row is printed once.`
