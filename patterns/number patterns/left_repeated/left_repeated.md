...
Print a left aligned triangle where row i contains the number i repeated i times.
...
The row index i is printed by the inner loop, so the value changes only when the row changes.
...
- Main concepts: Nested Loops, Left Triangle, Repeated Numbers.
