...
Print Floyd's triangle using a counter that increases after every printed number.
...
The counter starts at 1 and is incremented once per cell, so numbers continue row by row.
...
- Main concepts: Nested Loops, Counter, Floyd Triangle.
